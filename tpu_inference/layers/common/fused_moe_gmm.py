...
def gmm_wrapper(lhs,
    ...
    gmm_res = gmm_v2(
        ...
        rhs=rhs,
        rhs_scale=rhs_scale,
        # Per-quant-block affine bias (w = scale*q + groupbias). None for the
        # bf16 / symmetric-int paths (zero overhead, like rhs_scale=None).
...
def tensor_parallel_gmm(
    ...
    *,
    ...
    # groupbias rides the scale rails: same w1 spec (shard the out dim), and the
    # same block-dim-aware w2 rule computed from its OWN block count.
    ...
    w2_groupbias_spec = (None if w2_groupbias is None or num_gb_blocks == 1 else
...
def expert_parallel_gmm(
    ...
    *,
    ...
    # groupbias rides the scale rails: sharded on the leading expert axis, exactly
    # like scale.
...
def fused_moe_func(
    ...
    *,
    ...
    # hash-based and approx-topk are opt-in upstream routing overrides; the
    # default path uses hy3's compute_moe_routing, which preserves the
    # correction-bias selection / unbiased-weight / routed_scaling_factor and
    # the num_actual_tokens padding-mask (route padding rows to expert 0).
    if hash_based_topk_indices is not None:
        ...
        # Fold upstream's expert_score_correction_bias into hy3's
        # e_score_correction_bias selection path (both carry the same logical
        # per-expert correction bias from different call paths).
        correction_bias = (e_score_correction_bias
                           if e_score_correction_bias is not None else
                           expert_score_correction_bias)
