...
# vLLM relocated/renamed this layer across versions. Newer vLLM exposes it as
# `QwenGatedDeltaNetAttention` in `mamba.gdn.qwen_gdn_linear_attn`; older vLLM
# (e.g. the pinned checkout) exposes `GatedDeltaNetAttention` in
# `mamba.gdn_linear_attn`. Support both so model imports don't break across the
# pinned/nightly vLLM skew. The rest of this file uses the
# `QwenGatedDeltaNetAttention` name.
try:
