...
parameter layout (e.g. ``Qwen3MoeForCausalLM``).
...
DeepSeek-style router/shared-expert fixes (models whose MoE block carries a
quantized ``router.gate``, a ``router.expert_bias`` and a ``shared_mlp``):

  * ``...mlp.router.gate.{weight,scales,biases}`` is an 8-bit affine triplet
    (per-module quant override), but vLLM's gate linear is forced unquantized
  ...
  * ``...mlp.shared_mlp.{gate,up,down}_proj.{...}`` (the shared expert,
    4-bit) keeps its ``shared_mlp.`` infix -- vLLM models register the shared
    expert as ``self.shared_mlp``, so the param path is
    ``...mlp.shared_mlp.{gate,up,down}_proj.*`` (gate/up then merge into
    ``gate_up_proj`` via vLLM's stacked-params mapping). Stays 4-bit.
...
# Shared expert: keep the ``shared_mlp.`` infix (see module docstring);
# weight stays 4-bit-packed.
_SHARED = re.compile(
    r"^(.*)\.mlp\.shared_mlp\.(gate_proj|up_proj|down_proj)\.(weight|scales|biases)$"
)
# Router gate: 8-bit affine triplet, dequantized to bf16 at load.
...
def transform_mlx_weights(weights: Iterable[tuple[str, torch.Tensor]], *,
    ...
    for name, tensor in weights:
        ...
        if sh is not None:
            ...
            # Shared expert: KEEP the ``shared_mlp.`` infix -- vLLM models
            # register the shared expert as ``self.shared_mlp``, so the param
            # path is ``...mlp.shared_mlp.{proj}.{weight,scales,biases}``.
    ...
    # into the unquantized gate linear.
