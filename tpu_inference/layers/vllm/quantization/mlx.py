...
class VllmMLXConfig(QuantizationConfig, VllmQuantConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> "VllmMLXConfig":
        ...
        bits = config["bits"]
        # MLX per-module quant overrides appear as dict-valued entries keyed
        # by the full module path (e.g. ``model.layers.N.mlp.router.gate ->
        # {group_size, bits: 8}``). The only override supported is the 8-bit
        # router gate, which is dequantized to bf16 at load (the gate is
        # unquantized in vLLM). Validate fail-fast so an unhandled override
        # errors loudly instead of silently mis-loading.
    ...
    @classmethod
    def weight_stream_transform(cls, hf_config):
        """Loader hook: rewrite the raw MLX (name, tensor) checkpoint stream.

        MLX checkpoints stack the per-expert MoE weights and quantize
        embed/lm_head; transform_mlx_weights bridges them to the names/layout
        vLLM's MoE models expect. See get_weight_stream_transform in the
        quantization package __init__.
        """
        import functools

        from tpu_inference.layers.vllm.quantization.mlx_weight_transform import \
            transform_mlx_weights
        q = (getattr(hf_config, "quantization_config", None)
             or getattr(hf_config, "quantization"))
        return functools.partial(transform_mlx_weights,
                                 group_size=q["group_size"],
                                 bits=q["bits"],
                                 num_experts=hf_config.num_experts)

    @classmethod
    def ensure_load_format(cls, vllm_config) -> None:
        """Route MLX checkpoints through the streaming loader.

        Only the streaming loader applies the MLX weight-stream transform;
        under the default load_format="auto" vLLM would pick
        DefaultModelLoader and the un-transformed lm_head.biases / stacked
        switch_mlp tensors would reach the model and fail with an opaque
        shape error. Auto-select the streaming loader, keep dummy loads
        (which never read the checkpoint) untouched, and reject anything
        else loudly.
        """
        ...
        if load_format == "auto":
            ...
            vllm_config.load_config.load_format = "tpu_streaming_loader"
        elif load_format not in ("tpu_streaming_loader", "dummy",
                                 "pathways_dummy"):
            raise ValueError(
                "MLX checkpoints must load via the streaming loader "
                "(load_format='tpu_streaming_loader' or leave it 'auto'), "
