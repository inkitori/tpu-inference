...
class TPUWorker(WorkerBase):
    ...
    def shutdown(self) -> None:
        ...
        EngineCore.shutdown -> executor.shutdown -> worker.shutdown) expects
        the worker to drop its resources here; the base WorkerBase.shutdown is
        a no-op. On the JAX path the model params and KV cache live as JAX
        device arrays on the runner, and a plain `del LLM` does not collect
        those arrays promptly, so HBM stays pinned and a second in-process LLM
        fails its available-memory check. Delegate to the runner to delete
        them explicitly."""
