...
def test_synthetic_mlx_moe_logits_match_bf16_reference(tensor_parallel_size):
    ...
    with tempfile.TemporaryDirectory() as mlx_dir, \
        ...
        # IncrementalModelLoader.get_all_weights (the "tpu_streaming_loader"
        # load format); ensure_load_format would also select it from "auto",
        # but request it explicitly here to keep the synthetic test
...
def test_real_mlx_30b_auto_loader_serves_coherently():
    """A PLAIN ``LLM(model=...)`` with NO hardcoded load_format must serve
    the real MLX 30B coherently.

    This is the production path the synthetic test cannot exercise: with
    ``load_format="auto"`` (the default), ``VllmMLXConfig.ensure_load_format``
    (invoked from get_tpu_quantization_config) must force the streaming
    loader so the MLX weight transform runs (otherwise load crashes on
    lm_head.biases / un-unstacked switch_mlp). Greedy decode, one prompt,
    assert non-empty ASCII-coherent output. tp defaults to 8 (override via
    RUN_REAL_MLX_TP)."""
    from vllm import LLM, SamplingParams
