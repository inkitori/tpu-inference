...
"""Tests gmm_v2 per-group additive bias (rhs_groupbias) for affine quant.

Affine quant (e.g. MLX checkpoints) is ``w = scale * q + bias`` where ``scale`` and ``bias`` are
...
def test_gmm_v2_groupbias_w2_shaped_matches_affine_reference():
    ...
    not just the w13-shaped case above. Unquantized-lhs
    k-loop site: bit-exact affine equality vs a numpy reference.

    K=1536/gs=64 mirrors a down_proj contraction with 24 quant blocks;
    N=2048 is a representative hidden out_dim."""
